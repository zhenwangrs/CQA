...
def train(net):
    ...
    for epoch in range(model_config.num_epoch):
        ...
        dev_dataset = MyDataset(data_path=base_config.dev_data_path)
...
def evaluate(net):
    ...
    train_dataset = MyDataset(data_path=base_config.train_data_path)
    ...
    test_dataset = MyDataset(data_path=base_config.test_data_path)
    acc, f1, p, r = test(net, test_dataset)
    print('test acc:', acc, 'f1:', f1, 'p:', p, 'r:', r, datetime.now())

    askMen_dataset = MyDataset(data_path=base_config.askMen_data_path)
    acc, f1, p, r = test(net, askMen_dataset)
    print('askMen acc:', acc, 'f1:', f1, 'p:', p, 'r:', r, datetime.now())

    askWomen_dataset = MyDataset(data_path=base_config.askWomen_data_path)
